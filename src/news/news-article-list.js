...
export function renderNewsArticleList(controller) {
      ...
      <table class="table table-sm table-hover align-middle board-table mb-0">
        ...
        <tbody>
          ${pageArticles.map((a, relIdx) => {
            ...
            const titleText = (controller.showTranslation && a.translatedTitle) ? a.translatedTitle : a.title;
            const hasTranslation = !!a.translatedTitle;
            ...
            <tr class="${a.selected ? 'table-primary' : ''}">
              ...
              <td>
                <div class="d-flex align-items-center gap-2">
                  ${controller.showTranslation && hasTranslation ? '<span class="badge bg-success-subtle text-success border border-success-subtle small py-0 px-2 flex-shrink-0"><i class="bi bi-check-lg me-1"></i>번역됨</span>' : ''}
                ...
                ${controller.showTranslation && a.translatedTitle && a.translatedTitle !== a.title ? `
                  <div class="text-muted text-xs mt-1 fst-italic text-truncate ps-1">
                    <i class="bi bi-translate me-1"></i>${a.title}
                  </div>` : ''}
              </td>
              <td class="text-center">
  ...
  const paginationHtml = `
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 pt-3 px-1 border-top mt-2">
