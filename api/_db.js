  ...
  try {
    ...
    if (parseInt(postCount[0].count, 10) === 0) {
      await sql`
        ...
        VALUES 
          ('[공지] JBoard AdminLTE v4 및 Vercel 클라우드 시스템 오픈', '공지', '관리자', 'admin@jboard.local', '<p>안녕하세요! <strong>JBoard v4</strong>에 오신 것을 환영합니다.</p><p>Neon Postgres 및 Vercel Blob 스토리지, Quill 리치 텍스트 에디터가 연동된 정통 엔터프라이즈 게시판입니다.</p>', 128, 15, 2, '[]'::jsonb),
